...
const EdgeComponent = ({
    ...
    const actionLine = edge.action
        ? `${ACTION_LABEL[edge.action] ?? edge.action.toUpperCase()}`
        : null;

    const renderLabel = (x: number, y: number, dy: number) => (
        <text className={styles.edgeLabelText} x={x} y={y} dy={dy} textAnchor="middle">
            <tspan x={x} dy={actionLine ? "-0.5em" : "0"}>
                {symbolLine}
            </tspan>
            {actionLine && (
